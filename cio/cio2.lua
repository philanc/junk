...
function cio.newtask(f, timeout)
	...
	--    (starting at task creation)
	local deadline = timeout and (os.time() + timeout) or NODEADLINE
	local co = coroutine.create(f)
	local task = {co=co, deadline=deadline}
	return task
end --newtask
...
task status
	ok / running
	waiting
		waiting_time = sleeping
		waiting_io = reading or writing (or waiting_hangup?)
		waiting_signal ?   YAGNI
	error
		timeout
	done
