...
210924 
ple7 based on git ple. want to experiment on 
	- minimodes/menus
		how to define them in pleinit
	- backquote/F1 as alternate prefix / synonymous to ^X
		`s = `S = ^X^S  ( = f1-s )

1. 	esc and backquote defined as alias to ctlx prefix
	after ` or esc, lower-case letters converted to ctl-char
		so esc-a = ^X^A  (but esc-A = ^XA 
			-- assumed to be rarely/not used)
	-- keep backquote prefix? if yes must add `` = insert bq
		why bq?
		1. for kbd w/ bq, w/o esc ...hmm
		2. to allow bq-[  which doesn't work with esc ...hmm
...
function e.insertbq(b)
	-- insert a backquote (useful if bq used as prefix)
	return e.insch(b, 96)
end
...
editor.bindings = { -- actions binding for text edition
	...
	[27] = e.prefix_ctlx,	-- ESC (prefix, ^X alias - don't use with '[')
	[96] = e.prefix_ctlx,	-- backquote (prefix)
